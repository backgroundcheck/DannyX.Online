...
export const NeuralAnalyzer: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      <div className="flex flex-col gap-8">
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-[2.5rem] p-10 backdrop-blur-xl">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h2 className="text-xl font-black text-white uppercase tracking-tighter">Media Perception</h2>
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-[0.3em]">Deep Understanding Engine</p>
            </div>
          </div>

          <div 
            onClick={() => fileInputRef.current?.click()}
            className={`relative aspect-video rounded-3xl border-2 border-dashed transition-all cursor-pointer group flex items-center justify-center overflow-hidden
          ...
          <div className="mt-10">
            <button
              ...
              className="w-full py-5 bg-white text-black font-black uppercase tracking-[0.2em] text-[11px] rounded-full hover:bg-zinc-200 transition-all disabled:opacity-20 shadow-[0_0_30px_rgba(255,255,255,0.1)]"
            >
              {isAnalyzing ? 'Processing Visual Latents...' : 'Run Neural Analysis'}
            </button>
            <p className="mt-4 text-center text-[9px] text-zinc-600 uppercase tracking-widest font-bold">Powered by Gemini 3 Pro Multi-Modal Perception</p>
          </div>
        </div>
      ...
      <div className="flex flex-col min-h-[500px]">
        <div className="flex-1 bg-zinc-950 border border-zinc-800 rounded-[2.5rem] overflow-hidden relative shadow-2xl">
          ...
          <div className="p-10 h-full overflow-y-auto custom-scrollbar">
            {!analysis ? (
              <div className="h-full flex flex-col items-center justify-center text-zinc-800 text-center opacity-40">
                <div className="w-px h-20 bg-gradient-to-b from-transparent via-zinc-800 to-transparent mb-6" />
                <span className="text-[10px] uppercase tracking-[0.6em] font-black">Awaiting Semantic Data</span>
              </div>
            ) : (
              <div className="space-y-6 animate-in fade-in duration-700">
                <div className="flex items-center justify-between border-b border-zinc-900 pb-4 mb-8">
                  <span className="text-[10px] font-black text-purple-400 uppercase tracking-[0.5em]">Neural Readout v3.0</span>
                  <div className="flex gap-1">
                    {[1, 2, 3].map(i => <div key={i} className="w-1 h-1 bg-green-500 rounded-full animate-ping" style={{ animationDelay: `${i * 0.2}s` }} />)}
                  </div>
                </div>
                <div className="prose prose-invert prose-sm max-w-none text-zinc-400 font-mono leading-relaxed">
                  {analysis.split('\n').map((line, i) => (
